...
const DoctorsPage: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1); // Ant Design uses 1-based indexing
  const [pageSize, setPageSize] = useState(4);

  const { data: doctorsResponse, isLoading, error, isFetching } = useQuery({
    queryKey: ['doctors', currentPage - 1, pageSize], // Convert to 0-based for API
    queryFn: () => doctorsApi.getAllDoctors(currentPage - 1, pageSize),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });

  const doctors = doctorsResponse?.data?.content || [];
  const totalElements = doctorsResponse?.data?.page.totalElements || 0;
  console.log('Doctors Response:', totalElements);
    ...
    <MainLayout>
      <div className="bg-gradient-to-r from-teal-500 to-blue-500 py-8 md:py-12">
        <div className="container mx-auto px-4 sm:px-8 md:px-16 lg:px-32 xl:px-52">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-4">Danh sách bác sĩ</h1>
        ...
        {!isLoading && !error && (
          <>
            <div className="mb-4 text-gray-600 text-sm md:text-base">
              Hiển thị {doctors.length} trong tổng số {totalElements} bác sĩ
            </div>
