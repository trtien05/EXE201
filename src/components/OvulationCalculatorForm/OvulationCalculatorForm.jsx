...
const OvulationCalculatorForm = ({ onShowResult }) => {
  ...
  return (
    <div className="calculator-form">
      ...
      <label>Ngày đầu tiên của kỳ kinh cuối cùng của bạn</label>
      <Input
        type="date"
        value={startDate}
        onChange={(e) => {
        ...
        className={`input-field ${
          errors.startDate && formSubmitted ? "input-error" : ""
        }`}
      ...
      <label>Chu kỳ kinh nguyệt của bạn kéo dài bao lâu?</label>
      <Select
        value={cycleLength}
        onChange={(value) => {
      ...
      >
        {Array.from({ length: 22 }, (_, i) => {
          const days = 20 + i;
          return (
            <Select.Option key={days} value={days.toString()}>
              {days} days
            </Select.Option>
          );
        })}
