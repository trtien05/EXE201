 ...
 cung cấp thông tin chính xác và hữu ích. Mỗi lần trả lời chỉ nên có một câu hỏi hoặc một câu trả lời ngắn gọn.`;
  ...
  return (
    <>
      ...
      {!open && (
        <button
          className="fixed bottom-6 right-6 z-50 w-16 h-16 rounded-full bg-blue-500 flex items-center justify-center shadow-lg hover:bg-blue-600 transition-colors text-white"
      ...
      {/* Chatbot Box */}
      {open && (
        <div className="fixed bottom-6 right-6 w-[380px] h-[600px] z-50 flex flex-col justify-between shadow-xl rounded-xl bg-white border border-base-300 animate-fade-in">
          <div className="bg-primary text-primary-content px-4 py-3 rounded-t-xl font-bold flex items-center gap-2 justify-between">
            <span className="material-icons">Sencare AI</span>

            <button
              className="ml-auto text-black hover:text-gray-200 text-xl focus:outline-none"
